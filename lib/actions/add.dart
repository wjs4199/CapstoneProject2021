...
class _AddPageState extends State<AddPage> {
  ...
  Future<void> getImageFileFromAssets() async {
    ...
    for (var i = 0; i < imageList.length; i++) {
      path[i] = await FlutterAbsolutePath.getAbsolutePath(imageList[i].identifier);
      final byteData = await rootBundle.load('assets/$path');

      file[i] = File('${(await getTemporaryDirectory()).path}/$path');
      await file[i].writeAsBytes(byteData.buffer.asUint8List(byteData.offsetInBytes, byteData.lengthInBytes));
    }
  }
                                          ...
                                          Expanded(
                                            child: TextFormField(
                                              ...
                                              decoration: const InputDecoration(
                                                ...
                                                disabledBorder: InputBorder.none,
                                                hintMaxLines: 3,
                                                hintText: '당신이 나눔할 내용을 입력해주세요. 재사용이 불가능하거나 판매금지품목인 경우 게시가 제한될 수 있어요.',
